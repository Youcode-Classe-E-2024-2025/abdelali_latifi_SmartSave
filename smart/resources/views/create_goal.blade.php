    ...
    <div class="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md mt-8">
        ...
        <form action="{{ route('financial.storeGoal') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" required class="block w-full p-3 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500" placeholder="Entrez le nom de l'objectif">
            </div>

            <!-- Montant cible -->
            <div>
                <label for="target_amount" class="block text-lg font-medium text-gray-700 mb-2">Montant cible (€)</label>
                <input type="number" name="target_amount" id="target_amount" step="0.01" required class="block w-full p-3 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500" placeholder="Entrez le montant cible">
            </div>
